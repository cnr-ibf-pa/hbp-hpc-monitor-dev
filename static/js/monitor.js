let client = new jso.JSO({
    ...
    redirect_uri: "https://bspmonitors.cineca.it/",
...
function requestProject(session, hpc, isServiceAccount=false) {
    var URL = isServiceAccount ? URL = SERVICE_ACCOUNT_URL + "quotas/" + hpc.id + "/" : hpc.root_url;    
...
function checkJobSubmission(checkButton) {
    ...
    authorization.then((session) => {
        ...
        if (isServiceAccount) {
            ...
            if (hpcContext.id == "PIZDAINT") {
                URL = "https://bspmonitors.cineca.it/" + hpcContext.id.toLowerCase() + "/check";
                METHOD = "GET";
            } else {
...
function login() {
    ...
    authorization.then((session) => {
        $.ajax({
            url: "https://bspmonitors.cineca.it/user",
            headers: {
                "Authorization": "Bearer " + session.access_token
            },
            type: "GET",
